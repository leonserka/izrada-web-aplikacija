...
if os.environ['REQUEST_METHOD'].upper() == 'POST':
    ...
    for subject in subjects:
        ...
        if selected_status:
            ...
            db.add_upisni_list(session_data['user_id'], subject_id, selected_status) 
    session.add_to_session(session_data, session_id)

# Provjera koji je odabrani "year"
...
def print_submit_buttons():
    ...
    print('<a href="popis.py">Popis studenata</a>')
...
def print_subject_row(name, ects, selected_status):
    ...
    print("<td>")
    for status in ['not', 'enr', 'pass']: 
        checked = 'checked' if selected_status == status else ''
        print('<label><input type="radio" name="' + name + '" value="' + status + '" ' + checked + '> ' + predmeti.status_names.get(status) + '</label><br>')
    print("</td>")
    print("</tr>")
...
if selected_year in ['1', '2', '3']:
    ...
    for subject in subjects:
        if subject[3] == int(selected_year): 
            name = subject[1]
            ects = subject[2]
            selected_status = session_data.get(name, 'not')  
    ...
    for subject in subjects:
        name = subject[1]
        ects = subject[2]
        status = session_data.get(name, 'not')
        print("<tr>")
        print("<td>" + name + "</td>")
        print("<td>" + str(ects) + "</td>")
        print("<td>" + predmeti.status_names.get(status, 'Not selected') + "</td>")  # Prikaz statusa pomoću 'status_names'
        print("</tr>")
        if status == 'enr':
